...
sub calculateDbFreq {
    ...
    my $newInserts = 0;
    $cursor->immortal(1);
    my $germlineIndexes = $RunParameters->{'germline'}->indexes;
    my $tumorIndexes = $RunParameters->{'tumor'}->indexes;	
    ## Count per projectRun and files
    while ( my $doc = $cursor->next ) {
        $RunParameters->{'uidCollection'}->update_one( { $RunParameters->{'uidName'} => $doc->{ $RunParameters->{'uidName'} } },{'$set'=>{'inserted'=>int(0)}},{safe=>0});
               $germlineIndexes->create_many( { keys => ['variants.filepath' => 1]   },
					      { keys => ['variants.projectRun' => 1] },
					      { keys => ['aberration.aberration_filter' => 1] },
					      { keys => ['variants.projectRun' => 1, 'aberration.aberration_filter' => 1]},
					      { keys => ['type' => 1 ]} 
					    );
               $tumorIndexes->create_many(
					      { keys => ['variants.filepath' => 1]},
					      { keys => ['variants.projectRun' => 1]},
					      { keys => ['aberration.aberration_filter' => 1]},
					      { keys => ['variants.projectRun' => 1, 'aberration.aberration_filter' => 1]},
					      { keys => ['type' => 1]}
                ...
                $RunParameters->{'uidCollection'}->update_many( { $RunParameters->{'uidName'} => $doc->{ $RunParameters->{'uidName'} } },
                    ...
                    if ($#array>0) {
                       ...
                       $RunParameters->{'uidCollection'}->update_one( { $RunParameters->{'uidName'} => $doc->{ $RunParameters->{'uidName'} } },
                                                                  { '$set' => { 'medianMaf' => $med } },
                                                                  { safe => 1 }
                                                                );
                    }   
                foreach my $filepath ( @{ $doc->{'filepaths'} } ) {
                    ...
                    LOOP: while ( my $doc2 = $cursor2->next ) {
                       ...
                       $RunParameters->{$doc2->{'collectionDb'}}->indexes->create_one( [ 'variants.filepath' => 1 ] );
                        my $fileInsertCount = $RunParameters->{ $doc2->{'collectionDb'} }->count( { 'variants.filepath' => $filepath } );
                        $RunParameters->{'filesCollection'}->update_many( { '_id' => $doc2->{'_id'} }, { '$set' => { 'inserted' => $fileInsertCount } }, { safe => 1} );
    ...
    LOOP: while ( my $doc = $cursor->next ) {
       ...
       unless (exists($doc->{'collectionDb'})) {next LOOP}
        ...
        $RunParameters->{'filesCollection'}->update_many( { '_id' => $doc->{'_id'} }, { '$set' => { 'inserted' => $fileInsertCount } }, { safe => 1} );
        $RunParameters->{'uidCollection'}->update_one( { $RunParameters->{'uidName'} => $doc->{ $RunParameters->{'uidName'} } },
    ...
    for (my $i=0;$i<=$RunParameters->{'CollectionCount'};++$i) {
        ...
        my $fileConn = $RunParameters->{'mongoConnection'}->get_database( $RunParameters->{'DatabaseName'} )->get_collection('files');
	my $vcfColl = $RunParameters->{$vcfCollection};
	my $indexes = $vcfColl->indexes;
	$indexes->create_one( [ 'variants.filepath' => 1 ] );
#        $RunParameters->{$vcfCollection}->ensure_index( { 'variants.coord' => 1 } );
        $indexes->create_one( [ 'biomarker' => 1 ] );
	...
	my $indexes2 = $markerConn->indexes;
        $indexes->create_one(['info.checkCount'=>1]);
        if (exists($RunParameters->{'resetCount'})) {
            if ($RunParameters->{'resetCount'}==1) {
                $markerConn->update_many( { }, { '$set' => { 'info.checkCount' => int(1) } }, { 'safe' => 1 } );
            }
        }
        my %counted = ();
        my $superCount=0;
       RESET:
        my $cursor8 = $markerConn->find({'info.checkCount'=>1});
        $cursor8->immortal(1);
        my $counter = 0;
        my $totalCount=0;
      ...
      REQ: while ( my $doc = $cursor8->next ) {
	    ...
	    my $biomarkerOut; 
	    $biomarkerOut=$markerConn->aggregate([{'$match'=>{'biomarker' => $biomarker }},{'$group'=>{'_id'=>'biomarker','count'=>{'$sum'=>'$info.varArray'}}}]);
	    my $count;
	    if (is_array($biomarkerOut)) {
	     
	    	for my $elem (@$biomarkerOut) {   # lexical loop variable
    			$count = $elem->{'count'};
	    	}
	    	$biomarkerAlleleCount=int($count);
            }
...
#                }elsif ($freq>$doc->{'annotate'}->{'maxPopFreq'}) {
            ...
            $markerConn->update_many( { 'biomarker' => $biomarker },
                                 { '$set' => { 'dbFreq' => $freq,
                                               'dbFreqCat' => $cat,
...
sub cleanOld {
    ...
    while ( my $doc = $cursor0->next ) {
        ...
        if ( exists( $doc->{'collectionDb'} ) ) {
            ...
            delete $RunParameters->{'filesCollection'}->{ 'filepath' => $doc->{'filepath'} };
        }
    }
...
sub cleanAndMerge {
    ...
    for (my $i=1;$i<=$RunParameters->{'CollectionCount'};++$i) {
        ...
        my $connMarkers = $RunParameters->{'mongoConnection'}->get_database( $RunParameters->{'DatabaseName'} )->get_collection($collectionDb);
        my $indexes = $connMarkers->indexes;
	if (exists($RunParameters->{'largeRun'})) {
            if ($RunParameters->{'largeRun'}==1) {
                #print "\t+Reindexing $collectionDb\n";
                #$connMarkers->drop_indexes;
        ...
        $indexes->create_many(  keys => [ 'info.group' => 1 ],
        		       	keys => [ 'info.compact' => 1 ],
        			keys => [ 'info.group' => 1 ],
                                keys => [ 'info.toDelete'=>1 ],
                                keys => [ 'info.compact'  => 1],
                                keys => ['info.group' => 1, 'info.toDelete'=>1, 'info.varFilled'  => 1 ]
                             );
        print "\n\n+Starting merging for $collectionDb\n";
        my %foundGroup  = ();
        if (exists($RunParameters->{'resetCompact'})) {
            if ($RunParameters->{'resetCompact'}==1) {
                $connMarkers->update_many( { }, { '$set' => { 'info.compact' => int(0) } }, { 'safe' => 1 } );
            }
        }
      ...
      OUTERLOOP: while ( my $doc = $cursor->next ) {
            ...
            if (scalar(@{$out}) <1) {
                $connMarkers->updat_many( { '_id' => $doc->{'_id'} }, { '$set' => { 'info.compact' => int(1),'info.toDelete'=>int(0) } }, { 'safe' => 1 } );
                next OUTERLOOP;
            }
            my $biomarkerRef=$doc;
...
#                $connMarkers->update( { '_id' => $docFirst->{'_id'} }, { '$set' => { 'info.toDelete' => int($$) } }, { 'safe' => 1 } );
            ...
            my $newId = $connMarkers->insert_one( $biomarkerRef, { safe => 1 } );
            delete $connMarkers->{ 'info.toDelete' => int($$) };
            $connMarkers->update_one( { '_id' => $newId }, { '$unset' => { 'info.partialInsert' => "" } }, { 'safe' => 1 } );
        }
    }
    my $diff = localtime->hour * 3600 + localtime->min() * 60 + localtime->sec() - $start;
...
sub mongoConnect {
    ...
    #$RunParameters->{'locksCollection'}->indexes->create_one( [ 'bufferIndex' => 1 ] );
    #$RunParameters->{'filesCollection'}->indexes->create_one( [ 'filepath'    => 1 ] );
    for (my $i=0;$i<=$RunParameters->{'CollectionCount'};++$i) {
        my $collectionDb=$RunParameters->{'Collections'}->[$i]->{'CollectionName'};
    ...
    if ( exists( $RunParameters->{'bufferHost'} ) ) {
        for ( my $index = 0 ; $index <= $#{ $RunParameters->{'bufferHost'} } ; ++$index ) {
            if ( $RunParameters->{'locksCollection'}->count( { 'bufferIndex' => $index } ) == 0 ) {
                $RunParameters->{'locksCollection'}->insert_one( { 'bufferHost' => $RunParameters->{'bufferHost'}->[$index],
                                                               'status' => int(1),
                                                               'bufferIndex' => $index,
                                                               'hostname' => 'none',
                                                               'pid' => '0'
...
sub removeCollection {
    ...
    my $RunParameters = shift;
    $RunParameters = Maintain->reloadStartupRunParameters($RunParameters);
    delete $RunParameters->{'collectionConn'};
    delete $RunParameters->{'filesCollection'};
    return 1;
...
sub closeConnection {
    ...
    $RunParameters->{'locksCollection'}->update_one( { 'bufferIndex' => int( $RunParameters->{'bufferIndex'}->{$in} ) }, { '$set' => { 'status' => int(1), 'in' => "None", "hostname" => "none", "pid" => "None" } }, { safe => 1 } );
...
sub closeAllConnections {
    ...
    delete $RunParameters->{'locksCollection'};
...
sub loadLocations {
    ...
    while (<DB>) {
        ...
        chomp;
        my @fields = split( /\t/, $_ );
	#Changed in new gencode version
