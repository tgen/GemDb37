...
sub processVcfFile {
    ...
    if ( $RunParameters->{'filesCollection'}->count( { 'filepath' => $filepath } ) > 0 ) {
        ...
        $RunParameters->{'filesCollection'}->insert_one( { 'filepath' => $filepath, 'status' => int(0), 'pid'=>0 }, { safe => 1 } );
        sleep(1);
        ...
        unless (exists($holderRef->{'pid'})) {
            ...
            $RunParameters->{'filesCollection'}->delete_one({'pid'=>$$,'filepath' => $filepath});
            return 0;        
        }
    ...
    if ( $variantRef->{'varDatabase'} eq "PASS" ) {
        ...
        if ( exists( $variantRef->{ $RunParameters->{'uidName'} } ) ) {
            ...
            if (  $biomarkerRef->{'toAnnotateByCoord'} == 1 ) {
                $RunParameters->{'vcfCollection'}->{'nakedInsert_' . $biomarkerRef->{'collectionDb'}}->insert_one( $biomarkerRef, { safe => 1 } );
            }
            elsif (  $biomarkerRef->{'toAnnotateByGene'} == 1 ) {
                $RunParameters->{'vcfCollection'}->{'geneInsert_' . $biomarkerRef->{'collectionDb'}}->insert_one( $biomarkerRef, { safe => 1 } );
            }
...
sub parseVcfLine {
    ...
    } elsif ($variantRef->{'chr'} eq '.') {
       $variantRef->{'chr'} = 'fusion'
    }
...
sub parseInfo {
  ...
  WORD: foreach my $word (@words) {
        ...
        if ( $key eq "TYPE" && $val =~ /LOH/ ) {
           if (exists($RunParameters->{'IncludeLOHinInfoVariants'})) {
              if ($RunParameters->{'IncludeLOHinInfoVariants'} eq "1") {
              } elsif ($RunParameters->{'IncludeLOHinInfoVariants'} eq "0") {
                 $variantRef->{'varDatabase'} = "FAIL-LOH"; return -1; 
              } else {
                 die "\t\tPlease provide 0 or 1 for IncludeLOHinInfoVariants\n";
              }
           } else {
                 $variantRef->{'varDatabase'} = "FAIL-LOH"; return -1; 
           }
...
#            @list = `find $RunParameters->{'vcfPath'} -name "*.vcf"`;
            if (exists ( $RunParameters->{'mtime'})) {
               @list = `find -L $RunParameters->{'vcfPath'} -iname "*.vcf" -mtime $RunParameters->{'mtime'}`;
            }else{
                @list = `find -L $RunParameters->{'vcfPath'} -iname "*.vcf"`;
           }
...
sub scrubFilepath {
    ...
    if (exists($RunParameters->{'scrubEnabled'} ) ) { 
        if ($RunParameters->{'scrubEnabled'} eq "on" || $RunParameters->{'scrubEnabled'} eq "1" || $RunParameters->{'scrubEnabled'} eq "true") {
            ...
            $RunParameters->{$variantCollectionDb}->delete_many( { 'variants.filepath' => $mongoFilepath, 'info.varArray' => 1 }, { safe => 1, 'multiple' => 1 } );
            ...
            while (my $doc=$cursor->next) {
                ...
                $RunParameters->{$variantCollectionDb}->delete_many( { '_id'=>$doc->{'_id'},'variants' => { '$size' => 0 } }, { 'safe' => 1 } );                
            }            
            $RunParameters->{'filesCollection'}->delete_many( { 'filepath' => $mongoFilepath }, { safe => 1, 'multiple' => 1 } );
            $RunParameters->{'uidCollection'}->update_many( {}, { '$pull' => { 'filepaths' => { 'filepath' => $mongoFilepath } } }, { 'multiple' => 1, 'safe' => 1 } );
            $RunParameters->{'uidCollection'}->delete_many( { 'filepaths' => { '$size' => 0 } }, { 'multiple' => 1, 'safe' => 1 } );
...
sub CreateOnPath {
    ...
    if ( exists( $RunParameters->{'CreateOnPath'} ) ) {
        ...
        for (my $i=0;$i<=$RunParameters->{'CollectionCount'};++$i) {
            my $collectionName=$RunParameters->{'Collections'}->[$i]->{'CollectionName'};   
           $RunParameters->{$collectionName}->delete_many( { 'CreateOnPath' => $RunParameters->{'CreateOnPath'} }, { safe => 1, 'multiple' => 1 } );
           $RunParameters->{$collectionName}->delete_many( { 'CreateOnPath' => $RunParameters->{'CreateOnPath'} }, { safe => 1, 'multiple' => 1 } );    
        }            
        ...
        for (my $i=0;$i<=$RunParameters->{'CollectionCount'};++$i) {
            ...
            foreach my $file (@list) {
                ...
                chomp($file);
                $conn->delete_many( { $RunParameters->{ $RunParameters->{'uidName'} } => $RunParameters->{'uidName'}, 'varArray' => 1 },
                    { safe => 1, 'multiple' => 1 } );
                $conn->update(
                ...
                $RunParameters->{'filesCollection'}->delete_many( { 'filepath' => $file }, { safe => 1, 'multiple' => 1 } );
                $RunParameters->{'uidCollection'}
                  ->delete_many( { $RunParameters->{'uidName'} => $RunParameters->{ $RunParameters->{'uidName'} } }, { safe => 1, 'multiple' => 1 } );
            }
        }
        ...
        $RunParameters->{'uidCollection'}
          ->delete_many( { $RunParameters->{'uidName'} => $RunParameters->{ $RunParameters->{'uidName'} } }, { safe => 1, 'multiple' => 1 } );
        $RunParameters->{'filesCollection'}->delete_many( { 'CreateOnPath' => $RunParameters->{'CreateOnPath'} }, { safe => 1, 'multiple' => 1 } );
        $RunParameters->{'uidCollection'}->delete_many( { 'vcfPath' => $RunParameters->{'CreateOnPath'} }, { 'multiple' => 1, 'safe' => 1 } );
    }
}
