            ...
            for ( my $i = 0 ; $i <= $#{ $RunParameters->{'addedFilepaths'} } ; ++$i ) {
                ...
                if ( my $doc2 = $cursor2->next ) {
                    if ( $doc2->{'valid'} eq "1" ) {
                        ...
                        $RunParameters->{'filesCollection'}->update_many( { 'filepath' => $doc2->{'filepath'} },
                        ...
                        $RunParameters->{'uidCollection'}->update_one( { $RunParameters->{'uidName'} => $uid },
        ...
        foreach my $newRules ( keys %rulesSaw ) {
            ...
            MARKER: while ( my $biomarker = $biomarkerCursor->next ) {
                ...
                unless (exists ($biomarker->{ 'drug_rule_matched_flag'})) {
                    $conn->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_matched_flag' => int(0) } }, { 'safe' => 1 } );
                }
                ...
                if ( scalar(@finalMatchingRule) > 0 ) {
                    for (my $i=0;$i< $count;++$i) {
                        if ( exists( $finalMatchingRule[$i]->{'aberration_lookup'} ) ) {
                            $conn->update_one( { '_id' => $id }, { '$push' => { 'matching_rule' => $finalMatchingRule[$i] } }, { 'safe' => 1 } );
                        }
                    }
                    if ( $wildcard == 1 && $snv == 1 )    {
                         $conn->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_snv_wild_card_match'    => int(1) } }, { 'safe' => 1 } )
                    }
                    if ( $wildcard == 1 && $fusion == 1 ) {
                          $conn->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_fusion_wild_card_match' => int(1) } }, { 'safe' => 1 } )
                    }
                    $conn->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_matched_flag' => int(1) } }, { 'safe' => 1 } );
                }
                $conn->update_one( { '_id' => $id }, { '$set'  => { 'rules_checked'       => int(1) } },    { 'safe' => 1 } );
                $conn->update_one( { '_id' => $id }, { '$push' => { 'rules_checked_array' => $newRules } }, { 'safe' => 1 } );
            }
        }
        foreach my $newRules ( keys %rulesSaw ) {
            foreach my $filepath ( keys %lastFilepath ) {
                $RunParameters->{'mongoConnection'}->get_database( $RunParameters->{'DatabaseName'} )->get_collection('files') ->update_many(
                                                              { 'filepath' => $filepath },
                                                              { '$addToSet' => { 'rules_checked_array' => $newRules } }, { 'safe' => 1 }
                                                            );
            }
            foreach my $uid ( keys %lastUid ) {
                $RunParameters->{'uidCollection'}->update_many( { $RunParameters->{'uidName'} => $uid },
...
#                                my $biomarkerRef=$doc;
                               ...
                               $connStage->insert_one( $biomarkerRef, { safe => 1 } );
                               ++$counted;print "$coord $gene\n";
                               $conn->delete_one({ '_id' => $id} );
                          }
                        }
                    }
                }
...
sub runSnpeff {
  ...
  LOOP: while ( my $doc = $cursor->next ) {
    ...
    if ( $count > 0 ) {
        my $numRunning = 0;
	SLEEP: $numRunning=`ps -ef | grep "$RunParameters->{'Snpeff4Path'}" | grep snpEff\.jar | wc -l `;
        if ( length $numRunning ) {
		if ($numRunning > 1) {
                	sleep(60);
...
sub annotateSnpeff {
    ...
    $conn->ensure_index({'info.varArray' => 1});
    $connStage->ensure_index({'biomarker' => 1});
    $connStage->ensure_index({'info.group' => 1});
    $connStage->ensure_index({'info.varArray' => 1});
...
#                $biomarkerRef = dclone($doc);
    ...
    if ( -e ("$RunParameters->{'runDir'}/snpeff.$collectionDb.pid$$.an.tmp.vcf") ) 
	{ 
		#`rm $RunParameters->{'runDir'}/snpeff.$collectionDb.pid$$.an.tmp.vcf`;
	}
    if ( -e ("$RunParameters->{'runDir'}/snpeff.$collectionDb.pid$$.tmp.vcf") ) { 
		#`rm $RunParameters->{'runDir'}/snpeff.$collectionDb.pid$$.tmp.vcf`;
       }
    my $diff = localtime->hour * 3600 + localtime->min() * 60 + localtime->sec() - $start;
...
#  my $num=  total_size($biomarkerRef); print "$biomarkerRef->{'biomarker'}:$num\n";
               $RunParameters->{$collectionDb}->insert_one( $biomarkerRef, { safe => 1 } );
        }
    }
    $conn->remove();
    my $diff = localtime->hour * 3600 + localtime->min() * 60 + localtime->sec() - $start;
...
sub assignAberrationDocForSnvs {
    ...
    if (
        ...
        $reportRef->{'lookup'}            = join( "_", $reportRef->{'gene'}, $reportRef->{'aberration_type'} );
            ...
            $biomarkerRef->{'snpeff'}->{'functionalEffect'} =~/structural_interaction_variant/ ||
            $biomarkerRef->{'snpeff'}->{'functionalEffect'} =~/protein_protein_contact/
        ) {
        $reportRef->{'AminoAcidChange'}    = $biomarkerRef->{'snpeff'}->{'AminoAcidChange'};
        $reportRef->{'effect'}    = $reportRef->{'gene'} . "(" . $biomarkerRef->{'snpeff'}->{'AminoAcidChange'} . ")";
...
sub addSnpeffInfo {
    ...
    LOOP: for (my $k=0;$k<=$#snpeffs;++$k) {
        ...
        if (defined($effAr[7]) || $RunParameters->{'allSnpeffTranscripts'}==1 || $RunParameters->{'snpeffValidAnnotations'}->{'ALL'}==1) {
 	    ...
 	    #For AminoAcidChange, if the first one if structural_interaction_variant or protein_protein_interaction, then grab it from the next record
	    if ( $effAr[1] eq "structural_interaction_variant" || $effAr[1] eq "protein_protein_contact" ){
		for (my $z=0;$z<=$#snpeffs;++$z) {
                     my @annArAA = ( $snpeffs[$z] =~ /(.*?)\|/g );
		     my $aaChange = $annArAA[10];
		     if (  $aaChange =~ /p\.(.*)/ ) {
            ...
            elsif ( exists( $snpeffVariantRef->{'AminoAcidChange'} ) && exists($snpeffVariantRef->{'toJoinSnpeff'} )) {
