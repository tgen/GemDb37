...
sub joinVCF {
    ...
    LOOP: while (my $doc=$cursor->next) {
                        ...
                        unless (exists($genesLookup{$gene})) {next LOOP3};
                         ...
                         if ($insert==1) {                             
                             foreach my $filepathToAdd (@filepaths) {
                                     $markerConn1->update_many( Tie::IxHash->new('variants.filepath' => $filepathToAdd , 'gene' => $gene),
        ...
        close (FILE);
        $RunParameters->{'filesCollection'}->update_many({'filepath'=>$filepath},{'$set'=>{'joinGeneInMarkers'=>0}},{'safe'=>1});
    }
...
##################################
    ...
    elsif ( $fileInfoRef->{'filepath'} =~ /TGen\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?)\/(.*?\.vcf)$/ ) {
        $fileInfoRef->{'study'}                           = $2;
        $fileInfoRef->{'patient'}                         = $3;
        $fileInfoRef->{'collectionDate'}                  = $4;
        $fileInfoRef->{'sampleSubgroupVisitSampleNumber'} = $5;
        $fileInfoRef->{'assay'}                           = $6;
        $fileInfoRef->{'filename'}                        = $7;
        $fileInfoRef->{'style'}                        = 'reports';
  ...
  PATIENT: while ( my $doc = $cursor->next ) { 
   ...
   # $markerConn->ensure_index( Tie::IxHash->new('gene'=>1,'dbFreqCat' => 1, 'impact' => 1,'variants.genotype.s1.gt'=>1) );          
    ...
    foreach my $projectRunP ( reverse sort keys %projectRunPs ) {
        ...
        VARIANT: while ( my $doc = $cursor->next ) {
            ...
            if ( my $gene = $doc->{snpeff}->{gene} ) {
                ...
                for (my $m=0; $m<scalar(@{$doc->{variants}});++$m) {
                    ...
                    $markerConn1->update_one( { '_id' => $id, "variants.projectRun" => $projectRunP }, 
        ...
        $fileConn->update_many( {  'projectRun'=> $projectRunP }, { '$set' => { 'inheritanceCheck' => "yes" } }, { safe => 1} );
    }
    my $diff = localtime->hour * 3600 + localtime->min() * 60 + localtime->sec() - $start;
    ...
    for ( my $i = 0 ; $i <= $#{ $RunParameters->{'RulesFiles'} } ; ++$i ) {
        ...
        foreach my $newRules ( keys %rulesSaw ) {
          ...
          MARKER: while ( my $biomarker = $biomarkerCursor->next ) {
                ...
                $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_matched_flag' => int(0) } }, { 'safe' => 1 } );
                ...
                if ( scalar(@finalMatchingRule) > 0 ) {
                    if ( exists( $finalMatchingRule[0]->{'aberration_lookup'} ) ) {
                        $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$set' => { 'matching_rule' => \@finalMatchingRule } }, { 'safe' => 1 } );
                        if ( $wildcard == 1 && $snv == 1 )    { 
                             $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_snv_wild_card_match'    => int(1) } }, { 'safe' => 1 } ) 
                        }
                        if ( $wildcard == 1 && $fusion == 1 ) { 
                              $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_fusion_wild_card_match' => int(1) } }, { 'safe' => 1 } ) 
                        }
                        $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$set' => { 'drug_rule_matched_flag' => int(1) } }, { 'safe' => 1 } );
                    }
                }
                $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$set'  => { 'rules_checked'       => int(1) } },    { 'safe' => 1 } );
                $RunParameters->{$collectionDb}->update_one( { '_id' => $id }, { '$push' => { 'rules_checked_array' => $newRules } }, { 'safe' => 1 } );       
            }     
        }
        foreach my $newRules ( keys %rulesSaw ) {
            ...
            foreach my $uid ( keys %lastUid ) {
                $RunParameters->{'uidCollection'}->update_many( { $RunParameters->{'uidName'} => $uid, 'rules_checked_array' => { '$nin' => [$newRules] } }, 
