...
if (join(@ARGV)=~/--h/) {
    print <<OUT
    _____________GemDb3 $VERSION, Copyright 2014 TGen, David W. Craig_____________________
    ...
    Examples:
          ...
          ./GemDb3.pl CreateOnPath=/reports/C017/0001/20140609/T3/A1STX/analysisResults/ 
...
___________ STARTING GemDb3 $VERSION, Copyright 2014 TGen, David W. Craig ______
...
###############################################################
    unless ( -e "$RunParameters->{'runDir'}/$started" ) {
        print "\n+++-------------- Replacing a RUNNING file, did you mean to erase database.  Its still there-------------+++ \n";
