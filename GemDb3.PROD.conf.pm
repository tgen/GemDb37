...
###############################################################
        ...
        vcfPaths             => ["/labs/ngd-data/VCF", "/labs/ngd-data/reports"],
        mtime		     => "-5",
###############################################################        
    ###  HOST INFORMATION  #### Easy is to make all the same location
###############################################################
        MongodbHost    => "10.48.66.234:25755",
        AnnotateHost   => "10.48.66.234:25755",
        AnnotateDbName => "AnnotateBy",
        BufferHostList => [ { 'host'        => "10.48.66.234:25755/buffer", 'connections' => 960 } ], #Computationally intensive
...
###############################################################
        reportableGenes => {
            ...
            'K1ID2'    => "$bin/resources/db/tumor.txt",
            'K1RIB'    => "$bin/resources/db/tumor.txt",
...
###############################################################
		...
		collectionDb => 'tumor'
            ...
            'seuratPointMutationFile' => {
                regex        => '(seurat)|(allele)|(snpeff.fin).*.vcf',
                valid        => 1,
                collectionDb => 'tumor'
            },
           ...
           'lumosVarPointMutationFile' => {
            ...
            'GATK_Haplocaller' => {
                regex        => '(rnaHC_All)|(\.HC)|(ermline).*vcf$',
                valid        => 1,
                collectionDb => 'germline'
            },
            'GATK_UGFile' => {
